...
public class SikeService {
    ...
    public SikeEntity encode(SikeEntity sikeEntity) throws UnsupportedEncodingException {
        ...
        byte[] encodedMessageBytes = new byte[messageBytes.length];
        for (int i = 0; i < messageBytes.length; i++) {
            encodedMessageBytes[i] = (byte) (keyBytes[i%16] ^ messageBytes[i]);
        }

        sikeEntity.setEncodedMessage(new String(encodedMessageBytes));
        sikeEntity.setEncodedMessageBytes(encodedMessageBytes);

        return sikeEntity;
    }

    public SikeEntity decode(SikeEntity sikeEntity) {
        byte[] encodedMessageBytes = sikeEntity.getEncodedMessageBytes();

        SHAKEDigest shake = new SHAKEDigest(256);
        byte[] publicKeyBytes = sikeEntity.getKeyBytes();
        shake.update(publicKeyBytes, 0, publicKeyBytes.length);

        byte[] decodedMessageBytes = new byte[encodedMessageBytes.length];
        for (int i = 0; i < encodedMessageBytes.length; i++) {
            decodedMessageBytes[i] = (byte) (publicKeyBytes[i%16] ^ encodedMessageBytes[i]);
        }
